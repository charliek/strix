...
pub struct Theme {
    ...
    pub add_emph: Color,
    /// Neutral fill painted in the empty side-by-side column opposite a pure
    /// addition/deletion — a faint panel shade (~6% of `fg` blended into `bg`),
    /// the same colour on both sides regardless of which side is empty;
    /// inherit-when-unset.
    pub filler_bg: Color,
    pub del: Color,
    pub del_bg: Color,
...
struct ColorsFile {
    ...
    /// Unset falls back to the base preset's `filler_bg` — an existing user
    /// theme file predating this field is unaffected.
...
impl ColorsFile {
    fn apply(&self, theme: &mut Theme) {
        ...
        set(&self.filler_bg, &mut theme.filler_bg);
