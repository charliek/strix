...
//   0 Unified · 1 Side by side · 2 sep · 3 Line numbers · 4 Wrap lines ·
//   5 Cross-file scroll · 6 sep · 7 Changes panel · 8 sep · 9 Home · 10 History
...
fn open_view_menu_renders_rows_with_state_markers() {
    ...
    // off, cross-file scroll off, changes panel shown. Line numbers is row 3 (y=5),
    // Wrap lines row 4 (y=6), Cross-file scroll row 5 (y=7), Changes panel row 7 (y=9).
    let buf = render_buffer(&app, W, H);
    ...
    assert_eq!(
        ...
        " ",
        "cross-file scroll unchecked by default"
    );
    assert_eq!(cell_symbol(&buf, 9, 9), "[", "changes panel checkbox opens");
    assert_eq!(cell_symbol(&buf, 10, 9), "x", "changes panel checked");
}
...
fn keyboard_up_down_skips_separators_and_wraps() {
    ...
    key(&mut app, KeyCode::Down);
    assert_eq!(app.open_menu.unwrap().item, 5, "Cross-file scroll");
    key(&mut app, KeyCode::Down);
    assert_eq!(
        app.open_menu.unwrap().item,
        7,
        "skips separator 6 to Changes panel"
    );
    key(&mut app, KeyCode::Down);
    assert_eq!(app.open_menu.unwrap().item, 9, "skips separator 8 to Home");
    key(&mut app, KeyCode::Down);
    assert_eq!(app.open_menu.unwrap().item, 10, "History");
    key(&mut app, KeyCode::Down);
    assert_eq!(app.open_menu.unwrap().item, 0, "wraps to the top");
    key(&mut app, KeyCode::Up);
    assert_eq!(app.open_menu.unwrap().item, 10, "wraps back to the bottom");
}
...
fn clicking_inside_the_box_off_any_row_is_consumed() {
    ...
    open_menu(&mut app, VIEW_LABEL_X);
    // The box's bottom border (y=13) is inside its bounds but on no content row.
    click(&mut app, 15, 13);
    assert!(
...
fn review_session_view_menu_home_row_reads_review_and_is_checked() {
    ...
    // The Home row is full-index 9 → y=11; in a review session it is checked
    // (view == home == Review), so its radio is filled.
    let buf = render_buffer(&app, W, H);
    assert_eq!(
        cell_symbol(&buf, 10, 11),
        "●",
...
fn changes_panel_marker_tracks_show_changes() {
    ...
    let buf = render_buffer(&app, W, H);
    assert_eq!(cell_symbol(&buf, 9, 9), "[", "changes panel checkbox opens");
    assert_eq!(cell_symbol(&buf, 10, 9), "x", "checked while shown");
    key(&mut app, KeyCode::Esc);
    ...
    open_menu(&mut app, VIEW_LABEL_X);
    let buf = render_buffer(&app, W, H);
    assert_eq!(cell_symbol(&buf, 10, 9), " ", "unchecked while hidden");
}
...
fn activating_changes_panel_row_flips_show_changes_in_status_view() {
    ...
    key(&mut app, KeyCode::Down); // Cross-file scroll
    key(&mut app, KeyCode::Down); // skips separator -> Changes panel
    assert_eq!(app.open_menu.unwrap().item, 7, "landed on Changes panel");
    key(&mut app, KeyCode::Enter);
...
fn activating_changes_panel_row_does_not_persist_config() {
    ...
    key(&mut app, KeyCode::Down);
    key(&mut app, KeyCode::Down);
    key(&mut app, KeyCode::Down); // one more row now: land on Changes panel, not Cross-file scroll
    key(&mut app, KeyCode::Enter);
