...
//! `add_bg`/`del_bg` wash. Colours aren't visible in `dump_frame`'s glyph-only
//! text, so every assertion here reads the rendered `Buffer` via the shared
//! `TestBackend` colour helpers (`render_buffer`/`row_has_bg`).
