...
`filler_bg` colours the **empty column** in side-by-side mode when its
opposite has a pure addition or deletion (no partner line, so nothing to zip
it against): a single neutral tint, the same on both sides, instead of the
flat pane background, so the row still reads as "one side changed" at a
glance. A context pair (unchanged on both sides) and a modified pair (both
